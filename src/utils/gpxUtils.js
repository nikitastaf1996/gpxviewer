...
export const formatPace = (minPerKm) => {
  if (!minPerKm || isNaN(minPerKm) || minPerKm === Infinity) return '-';
...
export const parseGpxXml = (gpxXml) => {
  ...
  for (let i = 0; i < trkpts.length; i++) {
    ...
    const time = pt.getElementsByTagName('time')[0]
      ? new Date(pt.getElementsByTagName('time')[0].textContent)
      : null;

    let hdop = pt.getElementsByTagName('hdop')[0]
      ? parseFloat(pt.getElementsByTagName('hdop')[0].textContent)
      : null;
    if (!hdop) {
      const extensions = pt.getElementsByTagName('extensions')[0];
      if (extensions) {
        hdop = extensions.getElementsByTagName('hdop')[0]
          ? parseFloat(extensions.getElementsByTagName('hdop')[0].textContent)
          : null;
      }
    }
